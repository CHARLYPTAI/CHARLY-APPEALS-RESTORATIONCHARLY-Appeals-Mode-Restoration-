...
const Appeals: React.FC = () => {
  ...
  const loadProperties = async () => {
    try {
      setLoading(true);
      const response = await authenticatedRequest('http://localhost:8001/api/portfolio/');
      if (response.ok) {
  ...
  const renderTabContent = () => {
    switch (activeTab) {
      ...
      case 'evidence':
        return <EvidenceManagement selectedProperty={selectedProperty} valuation={valuation} setValuation={setValuation} />;
      case 'review':
...
const EvidenceManagement: React.FC<EvidenceManagementProps> = ({ 
  ...
  const handleUploadComplete = (uploadedData: any) => {
    console.log('📄 Document uploaded successfully:', uploadedData);
    
    // Auto-populate valuation fields if this is financial data
    if (uploadedData.financial_summary && valuation) {
      const updatedValuation = { ...valuation };
      
      if (uploadedData.document_type === 'income_statement') {
        // Update Income Approach with parsed data
        const summary = uploadedData.financial_summary;
        updatedValuation.incomeApproach = {
          ...updatedValuation.incomeApproach,
          grossRentalIncome: summary.gross_income || updatedValuation.incomeApproach.grossRentalIncome,
          operatingExpenses: summary.operating_expenses || updatedValuation.incomeApproach.operatingExpenses,
          netOperatingIncome: summary.net_operating_income || updatedValuation.incomeApproach.netOperatingIncome,
        };
        
        // Recalculate NOI and capitalized value
        const effectiveIncome = updatedValuation.incomeApproach.grossRentalIncome * (1 - updatedValuation.incomeApproach.vacancyRate);
        updatedValuation.incomeApproach.netOperatingIncome = effectiveIncome - updatedValuation.incomeApproach.operatingExpenses;
        updatedValuation.incomeApproach.capitalizedValue = updatedValuation.incomeApproach.netOperatingIncome / updatedValuation.incomeApproach.capitalizationRate;
        
        setValuation(updatedValuation);
        
        // Show success message
        alert(`📊 Income statement data automatically populated!\n\nGross Income: $${summary.gross_income?.toLocaleString()}\nOperating Expenses: $${summary.operating_expenses?.toLocaleString()}\nNOI: $${summary.net_operating_income?.toLocaleString()}`);
      }
      
      if (uploadedData.document_type === 'rent_roll') {
        // Update Income Approach with rent roll data
        const summary = uploadedData.financial_summary;
        updatedValuation.incomeApproach = {
          ...updatedValuation.incomeApproach,
          monthlyRent: summary.total_monthly_rent || updatedValuation.incomeApproach.monthlyRent,
          vacancyRate: (1 - summary.occupancy_rate) || updatedValuation.incomeApproach.vacancyRate,
          grossRentalIncome: (summary.total_monthly_rent * 12) || updatedValuation.incomeApproach.grossRentalIncome,
        };
        ...
        updatedValuation.incomeApproach.netOperatingIncome = effectiveIncome - updatedValuation.incomeApproach.operatingExpenses;
        updatedValuation.incomeApproach.capitalizedValue = updatedValuation.incomeApproach.netOperatingIncome / updatedValuation.incomeApproach.capitalizationRate;
        updatedValuation.incomeApproach.grmValue = updatedValuation.incomeApproach.monthlyRent * 12 * updatedValuation.incomeApproach.grossRentMultiplier;
        
        setValuation(updatedValuation);
        
        // Show success message
        alert(`🏢 Rent roll data automatically populated!\n\nMonthly Rent: $${summary.total_monthly_rent?.toLocaleString()}\nOccupancy: ${(summary.occupancy_rate * 100).toFixed(1)}%\nAnnual Income: $${(summary.total_monthly_rent * 12)?.toLocaleString()}`);
      }
    }
  };
  ...
  if (!selectedProperty) {
    return (
      <div style={styles.tabContent}>
        ...
        <p style={styles.sectionDescription}>
          Please select a property first to upload supporting documents.
        </p>
      </div>
    );
  }
  ...
  return (
    <div style={styles.tabContent}>
      <h3 style={styles.sectionTitle}>Evidence Management</h3>
      <p style={styles.sectionDescription}>
        Upload supporting documentation including income statements, rent rolls, and comparable sales data. 
        Financial documents will automatically populate valuation fields.
      </p>
      ...
      <DocumentUpload
        ...
        acceptedTypes={['.pdf']}
        maxFileSize={20 * 1024 * 1024} // 20MB for property documents
      />

      <div style={styles.evidenceHint}>
        <h4 style={styles.hintTitle}>💡 Smart Document Processing</h4>
        <p style={styles.hintText}>
          CHARLY automatically extracts financial data from your uploads:
        </p>
        <ul style={styles.hintList}>
          <li><strong>Income Statements:</strong> Gross income, operating expenses, NOI → Income Approach</li>
          <li><strong>Rent Rolls:</strong> Monthly rents, occupancy rates → Income calculations</li>
          <li><strong>Sales Data:</strong> Comparable properties → Sales Comparison Approach</li>
        </ul>
      </div>
    </div>
  );
...
const styles = {
  ...
  // Evidence Management Styles
  evidenceHint: {
    backgroundColor: `${APPLE_COLORS.BLUE}05`,
    border: `1px solid ${APPLE_COLORS.BLUE}20`,
    borderRadius: '8px',
    padding: SPACING.LG,
    marginTop: SPACING.LG,
  },
