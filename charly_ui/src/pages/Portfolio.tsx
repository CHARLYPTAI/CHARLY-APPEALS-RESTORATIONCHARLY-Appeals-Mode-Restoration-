...
const Portfolio: React.FC = () => {
  ...
  const handleDeleteProperty = async (propertyId: string) => {
    ...
    try {
      const response = await fetch(`http://localhost:8001/api/portfolio/properties/${propertyId}`, {
        method: 'DELETE',
      });
  ...
  const handleCreateAppeal = async (propertyId: string) => {
    try {
      const response = await fetch('http://localhost:8001/api/appeals/', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
      ...
      if (response.ok) {
        ...
        // Navigate to appeals page with the property ID
        navigate(`/appeals?property=${propertyId}`);
      } else {
        throw new Error('Failed to create appeal');
      }
    } catch (err) {
      console.error('Error creating appeal:', err);
      alert('Failed to create appeal. Please try again.');
    }
  };
