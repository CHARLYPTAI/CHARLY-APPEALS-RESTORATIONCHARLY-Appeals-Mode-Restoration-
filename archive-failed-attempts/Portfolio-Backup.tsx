...
export function Portfolio() {
  ...
  useEffect(() => {
    const initializeAuth = async () => {
      ...
      // Always attempt to load portfolio data
      try {
        await loadPortfolio();
      } catch (portfolioError) {
        console.error("Portfolio: Failed to load portfolio data:", portfolioError);
        // Continue - user can still add properties manually
      }
    };
  ...
  const { properties, loading, error, ingestFiles, addProperty, loadPortfolio } = usePortfolioStore();
  const {
    currentProperty,
    analysisResults,
    isAnalyzing,
  ...
  const handlePropertySelect = async (propertyId: string) => {
    ...
    try {
      ...
      // Call AI prediction model and narrative generation in parallel
      const [aiPredictionResponse, incomeResponse, salesResponse, costResponse] = await Promise.all([
        authenticatedRequest('/api/ai/predict/single', {
          method: 'POST', 
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            property_id: property.id,
            assessment_value: property.currentAssessment || 0,
            market_value: property.estimatedValue || 0,
            building_size: property.squareFootage || 0,
            age: property.yearBuilt ? (new Date().getFullYear() - property.yearBuilt) : 0,
      ...
      // Parse all responses
      const aiPrediction = await aiPredictionResponse.json();
      const incomeResult = await incomeResponse.json();
      const salesResult = await salesResponse.json();
      const costResult = await costResponse.json();

      // Combine AI prediction with narratives into comprehensive analysis result
      const comprehensiveResult = {
        property_id: propertyId,
        success_probability: aiPrediction.success_probability || 0.75,
        predicted_reduction: aiPrediction.predicted_reduction || 0.12,
        confidence_score: aiPrediction.confidence_score || 0.88,
        risk_score: aiPrediction.risk_score || 0.25,
        recommended_action: aiPrediction.recommended_action || "PROCEED",
        ...
        appeal_probability: aiPrediction.success_probability || 0.75,
        narratives: {
  ...
  const displayProperties: MockProperty[] = normalizeApiResponse(properties) as unknown as MockProperty[];
  ...
  const handleAddProperty = async () => {
    ...
    try {
      ...
      const propertyPayload = {
        ...
        state: 'Unknown', // TODO: Extract from data or set default
        zip_code: '00000', // TODO: Extract from data or set default
        county: newPropertyData.jurisdiction?.trim() || 'Default County', 
        property_type: mappedType,
        current_assessment: Number(newPropertyData.currentAssessment),
        assessed_value: Number(newPropertyData.currentAssessment),
        market_value: newPropertyData.estimatedValue 
          ? Number(newPropertyData.estimatedValue) 
          : Number(newPropertyData.currentAssessment) * 0.88,
                  ...
                  <Button 
                    ...
                    onClick={async () => {
                      ...
                      try {
                        ...
                        // Allow Supernova generation with basic property data - analysis optional for enhancement
                        if (!currentAnalysis) {
                          console.log('⚠️ No analysis data found, proceeding with basic property data for Supernova report');
                        }
